    ...
    case 38: {
      if (language === "en") {
        return [
          ...
          "4. If a core action from 【事件】 is completely absent -> warning.",
          "5. If the core event happens but in a fundamentally different setting/context -> warning.",
          "6. If 【节拍】 turning point is missing entirely -> warning.",
          "Severity: all warnings, never critical. Outline is a guide, not a rigid contract.",
        ].join("\n");
      }
      return [
        ...
        "4. 【事件】中某项核心动作完全缺失（不是重排、不是重新诠释——就是没有）→ warning。",
        "5. 核心事件发生了但发生在完全不同的场景/语境中 → warning。",
        "6. 【节拍】转折点完全缺席 → warning。",
        "严重度：一律 warning，不判 critical。细纲是创作指南，不是硬性合同。",
      ].join("\n");
    }
    case 34:
    case 35:
