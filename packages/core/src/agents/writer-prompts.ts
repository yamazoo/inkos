...
1. 以简体中文工作，句子长短交替，段落适合手机阅读（3-5行/段）。对话使用中文双引号""，禁止使用「」角引号
2. 目标字数：${lengthSpec.target}字，允许区间：${lengthSpec.softMin}-${lengthSpec.softMax}字
3. 伏笔前后呼应，不留悬空线；所有埋下的伏笔都必须在后续收回
4. 只读必要上下文，不机械重复已有内容
5. 正文禁止使用任何 markdown 格式标记（*斜体*、**粗体**、\`代码\`等）。视角切换段落直接写正文，不要用斜体标记
...
- 时间节奏多样性：禁止连续 3 章以上使用相同的时间锚点收尾（"夜深了""月光洒下""关灯躺下"）或开场（"清晨""天亮了""晨光透过"）。章末断章可以在任意时刻——午后对决中断、黄昏对话戛然而止、凌晨异响惊醒、正午阳光下的意外来客、暴雨将至的闷雷。时间本身就是悬念工具。当最近两章都落在夜晚时，第三章必须换锚点
...
本书基调为「冷冽中透出一线暖意」，爽点交付必须适配这一风格。以下手法适用于所有爽点类型：

**旁观者反应放大器**：用"安静→低语→死寂→突然一声"替代"哗然"。旁观者的沉默比喧哗更有力量。
例：「全场沉默了三秒。有人咽了口唾沫。然后一个声音从角落里飘出来——'这人……谁？'」

**评价逆转模板**：旧评价 → 新评价 → 终极评价。每次升级由一个具体动作触发，不用旁白解释。
例："废物" → "这人不简单" → "他是谁"

**情绪释放节拍**：压抑不超过 1500 字建议一次小释放——一个眼神变化、一句打破沉默的话、一个让旁观者愣住的动作。释放不需要大场面，一个细节就够。

**环境暗示替代直白情绪**：不说"他心中燃起希望"，而写"风突然停了。他抬起手。掌心里，那道裂痕正在愈合。"
...
- 【铁律】同一体感/意象禁止连续渲染超过两轮。第三次出现相同意象域（如"火在体内流动"）时必须切换到新信息或新动作，避免原地打转。章末 30% 尤其严格：前文写过的五感细节不要重述，同一情绪节拍展开不超过 1 轮，场景动作不循环。反例：✗ 章末用 67 行重复写"手冻伤、膝盖酸、后背凉" + 同一场景循环两轮。正例：✓ 老者看了他一眼，转身走了。拐杖声消失在拐角。陈渊站在原地，有什么东西微微动了一下。
...
本章正文严禁意识流泛滥。内心戏必须服务于情节推进，不能代替情节推进。

- **情绪声明句禁超频**：含"感到""心中涌起""心头一震""终于明白""终于意识到"等声明式情绪词的句子，每3000字最多3句。超出部分必须改写为动作外化——✗"他感到一阵恐惧"→✓"他的手指开始发抖，指甲掐进了掌心"
- **分析式推理句禁超频**：含"这意味着""权衡利弊""综合考虑""如果…那么"等分析框架词的句子，每3000字最多2句。超出部分必须改写为对话或直觉反应——✗"他心里盘算着，如果动手，风险在于对方修为远超自己"→✓"他攥紧了拳头，又慢慢松开"
- **独白段落不得连续堆叠**：连续3段及以上以内心活动为主（无对话、无动作、无场景描写）的段落即为违规。必须在第2段后插入对话、动作或场景切换
- **全章独白占比不超30%**：内心戏主导的段落总字数不得超过全章30%。超出时优先用对话和动作替代，而非简单删除

## 硬性禁令

- 【硬性禁令·最高优先】全文严禁出现"不是……而是……""不是……，是……""不是A，是B"句式，出现即判定违规。正反例：✗"他不是害怕，而是愤怒"→✓"他攥紧了拳，指节发白"；✗"这不是普通的剑，而是一柄杀过人的剑"→✓"剑身上有洗不掉的锈痕，凑近了闻，还有一丝血腥气"；✗"她不是在哭，而是在笑"→✓"她仰着头，肩膀一耸一耸的，嘴角咧到了耳根"。核心原则：用具体动作和细节替代抽象对比，让读者自己得出结论
- 【硬性禁令】全文严禁出现破折号"——"，用逗号或句号断句
- 【铁律】明喻（像/像是/宛如/犹如）全篇密度不超过每千字3个。超出时改用直接描写或动词驱动的画面。✗"他的目光像刀子" → ✓"他盯着对方，对方下意识退了半步"
- 正文中禁止出现hook_id/账本式数据（如"余量由X%降到Y%"），数值结算只放POST_SETTLEMENT

## 章尾铁律

- 【铁律】最后的正文段落（不含标题和分割线）必须包含至少一个外部锚点：具体动作（角色做出可观察的行为）、对话（引号内有人说话）、或环境细节（声音/光线/天气/空间变化）。禁止以纯心理活动、情绪声明或模糊预告收尾。
- 最后 3 段中，至少一段必须是外部事件（动作/对话/场景），不可全部是心理活动或氛围描写。
- 此规则与「80/20 断章」配合：断在 action-climax 的那一句本身就是外部锚点。
- ✅ 好结尾示例：
  - 他把断剑柄插进泥里，转身走了。（动作）
  - 远处传来一阵急促的脚步声。（环境）
  - "你确定？"对方没有回答。（对话）
...
function buildSatisfactionGuide(gp: GenreProfile, language: "zh" | "en"): string {
  ...
  if (typeEntries.length === 0 && gp.satisfactionTypes.length === 0) return "";
  ...
  const coldStyleNote = isEn
    ? `### Cold-Restrained Style Adaptation\nThis book's tone is "cold warmth" — satisfaction delivery must match this style:\n- Bystander reactions: silence → whisper → dead quiet → one sudden voice (NOT "everyone gasped")\n- Victory descriptions: restrained, internal, implied through environment (NOT triumphant monologue)\n- Environmental foreshadowing over explicit emotion: don't say "hope ignited in his heart" — write "the wind stopped. He raised his hand. The crack in his palm was healing."\n- Every 1500 words of pressure should include a micro-release: a glance shift, one sentence breaking silence, one action that stuns bystanders`
    : `### 冷冽风格爽点交付附加指引\n本书基调为「冷冽中透出一线暖意」，爽点交付必须适配这一风格：\n- 旁观者反应：安静→低语→死寂→突然一声（NOT "全场哗然"）\n- 胜利描写：克制、内敛、通过环境暗示（NOT 胜利独白）\n- 环境暗示替代直白情绪：不说"他心中燃起希望"，而写"风突然停了。他抬起手。掌心里，那道裂痕正在愈合。"\n- 压抑不超过 1500 字建议一次小释放：一个眼神变化、一句打破沉默的话、一个让旁观者愣住的动作`;

  return [header, intro, ...typeEntries, balanceNote, coldStyleNote].join("\n");
}
