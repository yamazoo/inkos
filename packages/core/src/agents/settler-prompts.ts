...
章节摘要是唯一被下游 Writer 用于理解"上章发生了何事"的依据。生成 chapterSummary 时必须：

1. **明确承接上章结果**：在 events 中必须写清楚"上章已知的 X → 本章的 Y 进展/转折"，例如"顾长命已知郑怀恩被追捕（第5章）→ 本章发现他身上有玄清阁玉牌"
2. **不遗漏跨章因果线索**：如果本章角色做了某个决定，而这个决定是上章事件的直接后果（如"因为上章发现了 X，所以本章去 Y 那里查"），必须在 events 中写明这条因果链
3. **stateChanges 也须跨章**：如果状态变化是连续积累的（如"玉简第二次发热"，前一次在第 N 章），在 stateChanges 中写明"延续自第 N 章的变化"
4. **events 最低要求**：events 字段必须包含至少 3 个具体事件点，每个点都应包含"因为上章 X → 本章 Y"或"本章新发生 Z"的结构

这样下游章节才能通过章节摘要理解剧情因果，而不是凭空出现新事件。`;
}
...
\`\`\`json
{
  ...
  "chapterSummary": {
    ...
    "title": "本章标题",
    "characters": "角色1,角色2",
    "events": "本章发生的具体事件（含上章到本章的因果承接：上章已知何事、本章有何进展或转折、关键冲突如何推进），不少于3句",
    "stateChanges": "本章引起的角色/世界状态变化（含跨章连续变化），不少于2句",
